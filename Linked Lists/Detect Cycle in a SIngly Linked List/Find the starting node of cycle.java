...
class MySLL extends SingleLinkedList{
	...
	Node findStartingNodeOfCycle(int size){
		Node horse, tortoise;
		//horse will take 2 steps , tortoise will take 1 step
		horse = tortoise = head;
		//given cycle already exists
		while(true){
			tortoise = tortoise.next;
		...
		while(tortoise.next != horse){
			tortoise=tortoise.next;
			count++;
		}
		horse = tortoise = head;
		// move the horse ahead by steps = number of nodes in the cycle
		for(int i = 0; i<count;i++){
			horse = horse.next;
		}
		int i = 0;
		//till both point to the same element then keep moving forward
		while(i<2*size && horse!=tortoise){
			horse = horse.next;
			tortoise = tortoise.next;
			i += 1;
		}
...
class Driver{
	...
	public static void main(String ...args){
		...
		System.out.println("Cycle exists : " + my_list.isCycle());
		if(my_list.isCycle()){
			Node starting_node = my_list.findStartingNodeOfCycle(size);
			System.out.println(starting_node.data);
		}
	} 
